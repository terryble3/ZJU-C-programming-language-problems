...
逐步替换。先替换内层k = FB((k*k+1+1),k)-7;再替换外层，k = k*k+1+1*k+1-7。代入k=3，得k=7。
...
程序的作用是对给定的一个数组从大到小排序，并将结果存放在另一个数组中，最后输出排序后数组，使用的是插入排序算法。
...
程序从main函数处开始执行，调用fun函数。fun函数读取一个字符，非回车则继续调用fun函数；直到最后一个fun函数读取到回车，这个fun函数才执行`putchar(c);`输出字符，此时这个函数执行完毕，调用这个函数的fun函数也进入`putchar(c);`这一步……整个程序相当于读入字符串倒序输出的效果。
